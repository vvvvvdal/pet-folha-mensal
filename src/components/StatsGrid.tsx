...
export function StatsGrid({ totalHours, activitiesCount, targetHours = 32 }: StatsGridProps) {
  ...
  return (
    <div className="p-4 rounded-xl border border-slate-800/80 bg-slate-900/40 mb-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-3">
        {/* Left: Summary numbers */}
        <div className="flex items-center gap-4">
          <div>
            <span className="text-2xl font-bold text-slate-100 tracking-tight">{totalHours}h</span>
            <span className="text-xs text-slate-400 ml-1">/ {targetHours}h meta</span>
          </div>
