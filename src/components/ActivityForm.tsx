...
export function ActivityForm({ onSave, editingActivity, onCancelEdit, defaultGatNumber = '04' }: ActivityFormProps) {
  ...
  const applyPreset = (type: 'gat' | 'geral') => {
    if (type === 'gat') {
      setDescription(`Reunião do GAT ${defaultGatNumber}`);
      setModality('Síncrona virtual');
    } else {
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        >
          ...
          <span>
            💡 <strong>Cálculo Automático PET:</strong> {start} às {end} = <strong>{previewHours} hora{previewHours === 1 ? '' : 's'}</strong> (regra de cada hora do relógio iniciada = 1h).
          </span>
        </div>
      </form>
    </div>
